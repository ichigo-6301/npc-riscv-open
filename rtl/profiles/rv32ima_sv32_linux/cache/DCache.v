...
module DCache #(
    ...
    input pipe_req_write,
    input [31:0] pipe_req_paddr,
    input [31:0] pipe_req_vaddr,
    input [31:0] pipe_req_wdata,
    input [3:0] pipe_req_wstrb,
    input [2:0] pipe_req_len,
    ...
    output reg [31:0] pipe_resp_rdata,
    output reg [31:0] pipe_resp_vaddr,
...
`ifdef NPC_CACHE_STAT
    ...
    output reg [63:0] stat_writeback_cycle
`endif
`endif
);
...
`endif
    ...
    localparam [3:0] S_MAINT_SCAN = 4'd12;
...
`endif
    ...
    reg pipe_resp_pending;
    reg pipe_read_way_r;
    reg pipe_req_write_r;
    reg [31:0] pipe_req_wdata_r;
    reg [3:0] pipe_req_wstrb_r;
    reg [31:0] pipe_req_vaddr_r;
    reg [31:0] pipe_req_seq_r;
    reg [15:0] pipe_req_epoch_r;
    reg pipe_req_exception_valid_r;
    reg [31:0] pipe_req_exception_cause_r;
    reg [31:0] pipe_req_exception_tval_r;
`ifdef NPC_ASIC
...
`endif
    ...
    wire maint_scan_last = (maint_set_r == INIT_LAST_SET) && maint_way_r;
    ...
    wire pipe_req_store_fast_ok = !pipe_req_write || write_back_en;
    wire dmem_wr_pipe_hit = pipe_req_fire && !pipe_req_exception_valid && !pipe_req_uncached &&
        pipe_req_hit && pipe_req_write && pipe_req_store_fast_ok;
    ...
    wire dmem_way = dmem_wr_from_refill ? victim_way_r :
        (dmem_wr_pipe_hit ? pipe_req_hit_way : cpu_req_hit_way);
    wire [SET_WORD_ADDR_BITS-1:0] dmem_waddr = dmem_wr_from_refill ? refill_set_word :
        (dmem_wr_pipe_hit ? pipe_req_set_word : cpu_req_set_word);
    wire [31:0] dmem_wdata = dmem_wr_from_refill ? refill_word_data :
        (dmem_wr_pipe_hit ? pipe_req_wdata : cpu_req_wdata);
    wire [3:0] dmem_wstrb_raw = dmem_wr_from_refill ? 4'b1111 :
        (dmem_wr_pipe_hit ? pipe_req_wstrb : cpu_req_wstrb);
    // Generate per-way write strobes directly. This keeps WEBWE generation
    // from depending on an extra shared enable->gate stage.
    wire dmem_wen_w0 = dmem_wr_from_refill ? (dmem_wr_refill && !victim_way_r) : (dmem_wr_hit && !dmem_way);
    wire dmem_wen_w1 = dmem_wr_from_refill ? (dmem_wr_refill &&  victim_way_r) : (dmem_wr_hit &&  dmem_way);
    ...
    wire dmem_rd_pipe = pipe_req_fire && !pipe_req_exception_valid && !pipe_req_uncached && pipe_req_hit && !pipe_req_write;
    ...
    wire dmem_rd_cmd_any = dmem_rd_pipe || dmem_rd_cmd || dmem_rd_wb;
    wire dmem_rd_way = dmem_rd_pipe ? pipe_req_hit_way :
        (dmem_rd_cmd ? cpu_req_hit_way : victim_way_r);
    wire [SET_WORD_ADDR_BITS-1:0] dmem_raddr = dmem_rd_pipe ? pipe_req_set_word :
...
`endif
    ...
    assign cpu_req_ready = cpu_can_fire && (!cpu_resp_valid || cpu_resp_ready) && !maint_flush_req;
    assign pipe_req_ready = (state == S_IDLE) && !maint_flush_req &&
        !cpu_resp_valid && !hit_resp_pending && !hit_bypass &&
        !pipe_resp_valid && !pipe_resp_pending &&
        !cpu_req_valid;
    ...
    wire [31:0] dmem_rd_result = (hit_resp_pending && hit_read_way_r) ||
        (pipe_resp_pending && pipe_read_way_r) ? dmem_rdata_w1 : dmem_rdata_w0;
    reg [31:0] resp_data;
    always @(*) begin
        if (hit_bypass)
...
`endif
            ...
            pipe_resp_pending <= 1'b0;
            pipe_read_way_r <= 1'b0;
            pipe_req_write_r <= 1'b0;
            pipe_req_wdata_r <= 32'b0;
            pipe_req_wstrb_r <= 4'b0;
            pipe_req_vaddr_r <= 32'b0;
            pipe_req_seq_r <= 32'b0;
            pipe_req_epoch_r <= 16'b0;
            pipe_req_exception_valid_r <= 1'b0;
            pipe_req_exception_cause_r <= 32'b0;
            pipe_req_exception_tval_r <= 32'b0;
            ...
            maint_set_r <= {SET_BITS{1'b0}};
            maint_way_r <= 1'b0;
            wb_for_maint_r <= 1'b0;
            maint_flush_done <= 1'b0;
`ifdef NPC_USE_DPI
...
`endif
            ...
            pipe_resp_rdata <= 32'b0;
            pipe_resp_vaddr <= 32'b0;
            pipe_resp_seq <= 32'b0;
...
`ifdef NPC_CACHE_STAT
            ...
            stat_writeback_cycle <= 64'd0;
`endif
`endif
        end else begin
            if (maint_flush_done && !maint_flush_req) begin
                maint_flush_done <= 1'b0;
            end
...
`ifdef NPC_USE_DPI
`ifdef NPC_CACHE_STAT
            if (cpu_req_fire || pipe_req_fire) begin
                stat_access <= stat_access + 64'd1;
                if (cpu_req_fire ? cpu_req_write : pipe_req_write) stat_store_access <= stat_store_access + 64'd1;
                else stat_load_access <= stat_load_access + 64'd1;
            end
            if (cpu_req_valid && !cpu_req_ready)
                stat_req_wait_cycle <= stat_req_wait_cycle + 64'd1;
            if (cpu_resp_valid && !cpu_resp_ready && (state == S_IDLE || state == S_REFILL_WAIT))
            ...
            if ((state == S_WB_REQ) || (state == S_WB_WAIT))
                stat_writeback_cycle <= stat_writeback_cycle + 64'd1;
...
`endif
                ...
                S_IDLE: begin
                    if (maint_flush_req && !maint_flush_done) begin
                        maint_set_r <= {SET_BITS{1'b0}};
                        maint_way_r <= 1'b0;
                        wb_for_maint_r <= 1'b0;
                        maint_flush_done <= 1'b0;
                        ...
                        pipe_req_write_r <= pipe_req_write;
                        pipe_req_wdata_r <= pipe_req_wdata;
                        pipe_req_wstrb_r <= pipe_req_wstrb;
                        pipe_req_vaddr_r <= pipe_req_vaddr;
                        pipe_req_seq_r <= pipe_req_seq;
                        pipe_req_epoch_r <= pipe_req_epoch;
                        pipe_req_exception_valid_r <= pipe_req_exception_valid;
                        pipe_req_exception_cause_r <= pipe_req_exception_cause;
                        pipe_req_exception_tval_r <= pipe_req_exception_tval;
                        pipe_resp_vaddr <= pipe_req_vaddr;
                        pipe_resp_seq <= pipe_req_seq;
                        pipe_resp_epoch <= pipe_req_epoch;
                        pipe_resp_exception_valid <= pipe_req_exception_valid;
                        pipe_resp_exception_cause <= pipe_req_exception_cause;
                        pipe_resp_exception_tval <= pipe_req_exception_tval;
                        pipe_resp_hit <= 1'b0;
                        pipe_resp_miss <= 1'b0;
                        if (pipe_req_exception_valid) begin
                            pipe_resp_rdata <= 32'b0;
                            pipe_resp_valid <= 1'b1;
                        end else if (pipe_req_uncached || !pipe_req_hit || !pipe_req_store_fast_ok) begin
                            pipe_resp_rdata <= 32'b0;
                            pipe_resp_miss <= 1'b1;
                            pipe_resp_valid <= 1'b1;
`ifdef NPC_USE_DPI
`ifdef NPC_CACHE_STAT
                            if (pipe_req_write) stat_store_miss <= stat_store_miss + 64'd1;
                            else stat_load_miss <= stat_load_miss + 64'd1;
                            if (pipe_req_uncached) stat_uncached_access <= stat_uncached_access + 64'd1;
`endif
`endif
                        end else if (pipe_req_write) begin
                            lru_mem[pipe_req_set] <= pipe_req_hit0 ? 1'b1 : 1'b0;
                            if (write_back_en) begin
                                if (!pipe_req_hit_way) dirty_mem_w0[pipe_req_set] <= 1'b1;
                                else                   dirty_mem_w1[pipe_req_set] <= 1'b1;
                            end
                            pipe_resp_rdata <= 32'b0;
                            pipe_resp_hit <= 1'b1;
                            pipe_resp_valid <= 1'b1;
...
`endif
                            ...
                            lru_mem[pipe_req_set] <= pipe_req_hit0 ? 1'b1 : 1'b0;
                            pipe_read_way_r <= pipe_req_hit_way;
                            pipe_resp_pending <= 1'b1;
                            pipe_resp_hit <= 1'b1;
`ifdef NPC_USE_DPI
`ifdef NPC_CACHE_STAT
...
`endif
                ...
                S_MAINT_SCAN: begin
                    if (write_back_en &&
                        (maint_way_r ? (valid_mem_w1[maint_set_r] && dirty_mem_w1[maint_set_r])
                                     : (valid_mem_w0[maint_set_r] && dirty_mem_w0[maint_set_r]))) begin
                        set_r <= maint_set_r;
                        victim_way_r <= maint_way_r;
                        victim_tag_r <= maint_way_r ? tag_mem_w1[maint_set_r] : tag_mem_w0[maint_set_r];
                        wb_word_r <= {WORD_BITS{1'b0}};
                        wb_for_maint_r <= 1'b1;
                        state <= S_WB_READ;
                    end else if (maint_scan_last) begin
                        maint_flush_done <= 1'b1;
                        state <= S_IDLE;
                    end else begin
                        if (!maint_way_r) begin
                            maint_way_r <= 1'b1;
                        end else begin
                            maint_way_r <= 1'b0;
                            maint_set_r <= maint_set_r + {{(SET_BITS-1){1'b0}}, 1'b1};
                        end
                    end
                end
                ...
                S_WB_WAIT: begin
                    if (lower_resp_fire) begin
                        ...
                        if (wb_for_maint_r) begin
                            wb_for_maint_r <= 1'b0;
                            if (maint_scan_last) begin
                                maint_flush_done <= 1'b1;
                                state <= S_IDLE;
                            end else begin
                                if (!maint_way_r) begin
...
`ifdef NPC_USE_DPI
            ...
            if (pipe_resp_pending && !pipe_resp_valid) begin
                pipe_resp_rdata <= dmem_rd_result;
                pipe_resp_valid <= 1'b1;
            end
            if (dmem_wen_w0) begin
                if (dmem_wstrb_w0[0]) data_mem_w0[dmem_waddr][7:0]   <= dmem_wdata[7:0];
                if (dmem_wstrb_w0[1]) data_mem_w0[dmem_waddr][15:8]  <= dmem_wdata[15:8];
                if (dmem_wstrb_w0[2]) data_mem_w0[dmem_waddr][23:16] <= dmem_wdata[23:16];
                if (dmem_wstrb_w0[3]) data_mem_w0[dmem_waddr][31:24] <= dmem_wdata[31:24];
            end
            if (dmem_wen_w1) begin
                if (dmem_wstrb_w1[0]) data_mem_w1[dmem_waddr][7:0]   <= dmem_wdata[7:0];
                if (dmem_wstrb_w1[1]) data_mem_w1[dmem_waddr][15:8]  <= dmem_wdata[15:8];
                if (dmem_wstrb_w1[2]) data_mem_w1[dmem_waddr][23:16] <= dmem_wdata[23:16];
                if (dmem_wstrb_w1[3]) data_mem_w1[dmem_waddr][31:24] <= dmem_wdata[31:24];
            end
`endif
        end
    end
...
`ifndef NPC_USE_DPI
    // FPGA backend: TDPRAM (True Dual Port RAM) for DCache
    // Port A: write (store hits + refill data)
    // Port B: read  (CPU loads + writeback reads)
    xpm_memory_tdpram #(
        ...
        .douta(), .doutb(dmem_rdata_w0),
        .addra(dmem_waddr), .addrb(dmem_raddr),
        .clka(clk), .clkb(clk),
        .dina(dmem_wdata), .dinb(32'b0),
        .ena(1'b1), .enb(dmem_ren_w0),
        .wea(dmem_wstrb_w0), .web(4'b0000)
    );
    ...
    xpm_memory_tdpram #(
        ...
        .douta(), .doutb(dmem_rdata_w1),
        .addra(dmem_waddr), .addrb(dmem_raddr),
        .clka(clk), .clkb(clk),
        .dina(dmem_wdata), .dinb(32'b0),
        .ena(1'b1), .enb(dmem_ren_w1),
        .wea(dmem_wstrb_w1), .web(4'b0000)
    );
`endif
