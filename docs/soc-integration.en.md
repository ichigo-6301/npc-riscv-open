...
`reference-only` means that a software/model address layout can be consulted.
It does not mean the public CPU wrapper instantiates that AXI IP or that RTL
interrupt and driver behavior has been validated.
...
The current local NEMU adapter is a bounded GPR/PC/commit checker. It does not
check UART, AXI Timer, cycle-exact ACLINT ticks, AXI INTC, complete interrupt
timing, or arbitrary MMIO side effects. The timer models do not claim
