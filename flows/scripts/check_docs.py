...
OOO_HISTORY_CLAIM_ANNOTATIONS = (
    ...
    ("ooo_historical_coremark_approx_speedup", "partial", "5.633830352325",
     "pre_combinational_loop_remediation"),
    ("ooo_historical_comb_loop_zero", "partial", "0",
     "d12_registered_causal_ownership"),
    ("ooo_historical_unoptflat_zero", "partial", "0",
     "d12_registered_causal_ownership"),
...
OOO_HISTORY_REQUIRED_TEXT = {
    "docs/evidence/ooo_architecture_performance_history.md": (
        ...
        "两端退休指令相差 12 条。",
        "仅为 report-only `historical_partial`；",
        "仅为 log-only `historical_partial`；",
        "因此这两项 为 `historical_verified`。",
    ...
    "docs/evidence/ooo_architecture_performance_history.en.md": (
        ...
        "so it is report-only `historical_partial`;",
        "so it is log-only `historical_partial`;",
        "so both SCC results are `historical_verified`.",
